...
# by Jens 2014-09-08
# Python 3 version 2023-05-23
# Glyphs version 2025-11-25
...
from GlyphsApp import drawingTools as drawing
from GlyphsApp.UI.CanvasView import CanvasView
...
class UnitSystem(object):
    def __init__(
        self, name, units, min_units=None, max_units=None, strategy="free", unit_dict={}
    ):
        self.name = name
        self.upm = units
...
class HistogramUI:
    def __init__(self):
        ...
        self.w = vanilla.FloatingWindow(
            (window_width, window_height), "Glyph Width Histogram"
        )
        self.w.histogram = CanvasView(
            (histogram_x_offset, histogram_y_offset, -10, self.histogram_height),
            self,
        ...
        self.w.unit_label = vanilla.TextBox(
            (histogram_x_offset, y, 90, 20), "Units per em:"
        )
        self.w.unit_display = vanilla.TextBox(
            (histogram_x_offset + 90, y, 30, 20), self.units
        )
        self.w.system_selection = vanilla.PopUpButton(
            (histogram_x_offset + 120, y - 1, 300, 20),
            [],
            callback=self._systemCallback,
            # sizeStyle = "small",
        )
        self.w.show_fixed = vanilla.CheckBox(
            (histogram_x_offset + 462, y - 1, 300, 20),
            "Show system widths",
            value=self.show_fixed,
            callback=self._showFixedCallback,
        )
        y += 28
        self.w.unit_slider = vanilla.Slider(
            (histogram_x_offset, y, -10, 20),
            minValue=1,
            maxValue=64,
            value=self.units,
            tickMarkCount=64,
        ...
        y += 33
        self.w.glyph_selection_label = vanilla.TextBox(
            (histogram_x_offset, y, 120, 20), "Analyze glyphs:"
        )
        self.w.glyph_selection = vanilla.RadioGroup(
            (histogram_x_offset + 110, y, 210, 20),
        ...
        self.w.charset_selection = vanilla.PopUpButton(
            (histogram_x_offset + 320, y, -110, 20),
            [],
            callback=self._charsetCallback,
            # sizeStyle = "small",
        )

        self.w.update_button = vanilla.Button(
            (-100, y, -10, 20), "Update", callback=self._updateButtonCallback
        )
        ...
        if self.units in self.known_units:
            ...
            self.w.system_selection.set(
                unitization_info.get_ui_list().index(self.system.name)
            )
        else:
            self.system = None
    ...
    def get_glyphnames_for_histogram(self):
        ...
        if mode == 0:
            ...
            names = [layer.parent.name for layer in font.selectedLayers]
        elif mode == 1:
            ...
            all_glyphs = font.glyphs.keys()
            selected_charset_name = self.w.charset_selection.getItems()[
                self.w.charset_selection.get()
            ]
            names = [
                name
                for name in self.charsets[selected_charset_name]
    ...
    def calculate_histogram(self, sender=None):
        ...
        try:
            ...
            for name in self.glyphs:
                width = font.glyphs[name].layers[master].width
                if width > max_width:
                    max_width = width
    ...
    def draw(self, view):
        ...
        if self.show_fixed and self.system is not None and self.system.fixed_units:
            ...
            self.draw_histogram(
                ...
                (0, 0, 1, 0.5),
                True,
                histogram=self.system.fixed_units,
            )
        # draw the histogram for the current font
        self.draw_histogram(font, font.upm, (1, 0, 0, 1), True)
    ...
    def draw_histogram(self, font, upm, color, show_glyphs=False, histogram=None):
        ...
        for width in sorted(histogram.keys()):
            ...
            if show_glyphs:
                ...
                for i in range(len(histogram[width])):
                    ...
                    if glyph_name in font.glyphs:
                        c = font[glyph_name].string
                        if c:
                            drawing.text(
                                "%s" % c, (x + 4, 18 + i * self.scale_vertical)
                            )
                        else:
                            drawing.text(
                                "%s" % glyph_name, (x + 4, 18 + i * self.scale_vertical)
                            )
                    else:
                        drawing.text(
                            "%s" % glyph_name, (x + 4, 18 + i * self.scale_vertical)
            ...
            drawing.text(
                "%s" % (num), (x - 3 * len(str(num)), 22 + num * self.scale_vertical)
            )
            drawing.restore()
        drawing.restore()
