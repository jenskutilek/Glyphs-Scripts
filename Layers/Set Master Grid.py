...
class GridDialog(object):
    def __init__(self):
        self.w = vanilla.Window((300, 160), "Master Grid",)
        y = 8
        self.w.master_name = vanilla.TextBox(
            (8, y, -8, 17), "Set local grid for master: None"
        )
        ...
        self.w.button_cancel = vanilla.Button(
            (-272, -30, -204, -10), "Cancel", callback=self.callback_cancel,
        )
        ...
        self.w.button_set = vanilla.Button(
            (-84, -30, -8, -10), "Set Grid", callback=self.callback_set,
        )
        self.update()
        self.w.open()
        self.w.makeKey()
    ...
    def update(self):
        ...
        if self.master is None:
            ...
            self.w.master_name.set(
                "Set local grid for master: %s" % self.master.name
            )
